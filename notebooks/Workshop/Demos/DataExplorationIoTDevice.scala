...
// DBTITLE 1,Exploratory Analysis of Data Emitted by IoT Devices
// MAGIC %md ## Processing IoT Device JSON Data Using Datasets
...
// MAGIC 
// MAGIC This notebook, shows how you read a JSON file, convert your semi-structured JSON data into a collection of Datasets[T], where `T` is a class and introduce some high-level Spark 2.0 Dataset APIs.
...
// MAGIC * Battery  level of devices
// MAGIC * Humidity and Temperature of the location
// MAGIC * Carbondioxide Levels (CO2)
// MAGIC * Latitude and longitude
// MAGIC * etc..
// MAGIC 
...
// MAGIC %md  Apache Spark creates an optimized logical and physical plan for execution, and optimizes the generated code. For our `DeviceIoTData`, it will use its standard encoders to optimize its binary internal representation, hence decrease the size of generated code, minimize the bytes transferred over the networks between nodes, and execute faster.
// MAGIC 
...
// MAGIC 
// MAGIC Try changing the values for battery_level and CO2 levels and see the changes in the results.
